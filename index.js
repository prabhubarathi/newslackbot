...
function handleMessage(message) {
    ...
    if(message.includes(' how many entities do i have in NI')) {
        entitylookup();
    }
    ...
    if(message.includes(' NewAppTier')){
        apptier();
    }
}
...
function entitylookup() {
    ...
    axios.get('https://www.mgmt.cloud.vmware.com/ni/api/ni/entities/vms/', config)
        .then(res => {
            const entity = res.data.results[1];
        bot.postMessageToChannel('general',`${entity}`);
    }
)
}
